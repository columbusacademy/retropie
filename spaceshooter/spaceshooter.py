...
def update(time_interval):
    ...
    if status == "start-wait":
        if (time.time() < wait_timer):
            return
        if keyboard.space or keyboard.lshift or joystick.get_button(1)==1: #button 1 is upper row closest to joystick
            player.reset()
            enemies.reset()
            status = "game"
        ...
        if (time.time() < wait_timer):
            return
        if keyboard.space or keyboard.lshift or joystick.get_button(1)==1:
            status = "start"
    elif status == "game":
        # Scroll screen
